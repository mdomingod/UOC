...
from src.project_package.evaluate_complex          import (
...
def save_reports(run_dir: Path, cfg: Config, report: dict, train_info: dict):
    ...
    # 3. Per-primer table (large, save report['primer_table'].to_csv(run_dir / "primer_table.csv", index=False)as CSV for downstream analysis)
    
    if 'panel_table' in report:
        report['panel_table'].to_csv(run_dir / "panel_table.csv", index=False)

    # 4. Human-readable Markdown report
    md = render_markdown_report(cfg, report, train_info)
...
def main():
    ...
    """if cfg.test_panels:
        train_idx, val_idx, test_idx = split_by_panel(
            primer_ids   = data["primer_ids"],
            panel_ids    = data["panel_ids"],
            test_panels  = cfg.test_panels,
            val_fraction = cfg.val_fraction,
            seed         = cfg.seed,
        )
    else:"""

    # Fallback to primer-level split if no test panels designated
    ...
    train_seqs = set(data['primer_seqs'][train_idx])
    val_seqs = set(data['primer_seqs'][val_idx])
    ...
    print(f"  Unique primers in train: {len(train_seqs):,}")
    print(f"  Unique primers in val:   {len(val_seqs):,}")
    ...
    # ---------- Load the model to evalate ----------------------
    print("[3/5] Loading the model trained...")
    model.load_state_dict(torch.load(
        "/home/domingom/projects/SPE_Primer_Alignments/reports/" \
        "2026-06-01_012620_baseline_DHS001Z_SplitByPrimer_tv04_bs600_ep50_lr1e-4/model.pt"))
    ...
    if y_pred_denorm.std() > 0 and y_true_denorm.std() > 0:
        pearson_r = pearsonr(y_pred_denorm, y_true_denorm)
        spearman_r = spearmanr(y_pred_denorm, y_true_denorm)
    else:
        pearson_r, spearman_r = float('nan'), float('nan')

    df_val['pearson_r'] = pearson_r
    df_val['spearman_r'] = spearman_r """

    df_val.to_csv(run_dir / f"{cfg.run_name}_validation_predictions.csv", index=False)

    # Plots 
    plot_pred_vs_true_fig = plot_pred_vs_true(y_true_denorm, y_pred_denorm, "Predicted vs true (normalised)")
    save(plot_pred_vs_true_fig, run_dir, f"{cfg.run_name}_plot_pred_vs_true.png")

    plot_residuals_fig = plot_residuals(y_true_denorm, y_pred_denorm, "Residuals (normalised)")
    ...
    # Log primer-level metrics (skip NaN values — MLflow rejects them)
    primer_metrics = report['primer_level']
    for k, v in primer_metrics.items():
        if isinstance(v, (int, float)) and not (isinstance(v, float) and np.isnan(v)):
            mlflow.log_metric(f"primer_{k}", v)


    # Save report
    save_reports(run_dir, cfg, report, train_info)
    ...
    test_data = {
        ...
        "primer_ids": data["primer_ids"][test_idx],
    }
    if "is_target" in data:
        test_data["is_target"] = data["is_target"][test_idx]
    ...
    print()
    print(f"  Site-level MSE:      {site_metrics['mse']:,.2f}")
    print(f"  Site-level Pearson:  {site_metrics['pearson']}")
    print(f"  Site-level Spearman: {site_metrics['spearman']}")
    if 'primer_level' in report:
        print(f"  Primer-level MAPB:   {report['primer_level']['mean_abs_bias']}")
    print("""
